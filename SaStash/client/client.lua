...
local filterArray_main_crack = {Config.Lang.filterArray_main1, Config.Lang.filterArray_main2, Config.Lang.filterArray_main3, Config.Lang.filterArray_crack_open};
local filter_main_crack = 1;
...
AddEventHandler('SAM:OpenByTarget', function(Stat)
    if exports.ox_inventory:GetItemCount(Config.LockpickItems) > 1 then
        OpenMenuStash(Stat.name, Stat.coords.x, Stat.coords.y, Stat.coords.z, true)
    else
        OpenMenuStash(Stat.name, Stat.coords.x, Stat.coords.y, Stat.coords.z, false)
    end
end)
                            ...
                            if DelElevator == "YES" then
                                for k, v in pairs(AllStashIni) do
                                    if v.name == GestionName then 
                                        ...
                                        TriggerServerEvent('SAM:removeStashIni', v.id, invid, v.label)
                                    end
                                end 
...
function OpenMenuStash(Name, px, py, pz, lockpick)
    if MenuStash then
        MenuStash = false
    else
        MenuStash = true
                            ...
                            if lockpick then
                                RageUI.List(v.label, filterArray_main_crack, filter_main_crack, nil, {RightLabel = ""}, true, function(h, a, s, i)
                                    ...
                                    if s then
                                        ...
                                        elseif i == 4 then
                                            local RandomCode = {math.random(0,99), math.random(0,99), math.random(0,99)}
                                            DebugPrint(RandomCode[1], RandomCode[2], RandomCode[3])
        ...
        if #AllStash < 1 then
            ...
            for k, v in pairs(AllStash) do
                ...
                --if ESX.PlayerData.job and ESX.PlayerData.job.name == v.Job and MenuElevator == false then 
                    --DebugPrint(Config.RangeDrawMarker == false, not type(Config.RangeDrawMarker) == "number")
                    ...
                    if dist <= Config.RangeActive and not isMinigame then
                        wait = 0
                        ESX.ShowHelpNotification(Config.Lang.Help_notif, true, false) 
                        if IsControlJustPressed(1,51) then
                            DebugPrint(exports['ox_inventory']:GetItemCount(Config.LockpickItems))
                            if exports['ox_inventory']:GetItemCount(Config.LockpickItems) > 0 then
                                OpenMenuStash(v.name, v.coords.x, v.coords.y, v.coords.z, true)
                            else
                                OpenMenuStash(v.name, v.coords.x, v.coords.y, v.coords.z, false)
                            end
                        end
...
function DebugPrint(...)
    ...
    if Config.Debug_Print == 'traceback' then
        print('___________________________________________')
