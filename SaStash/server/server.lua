...
function Refresh()
    ...
    MySQL.Async.fetchAll("SELECT * FROM SaStashIni", {}, function(data)
        for i = 1, #data, 1 do
            exports['ox_inventory']:RegisterStash("SaStash-" .. data[i].name .. data[i].id, data[i].label, data[i].slot, data[i].weight, "")
...
AddEventHandler('SAM:sendToDiscordWithSpecialURL', function(title, message, color, url, wait)
    wait = wait or 0
    Wait(wait)
    sendToDiscordWithSpecialURL(title, message, color, url)
end)
